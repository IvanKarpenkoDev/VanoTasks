...
async def get_project_by_id(id: int, session: AsyncSession = Depends(get_async_session)):
...
async def get_projects_by_id(user_id: int, session: AsyncSession = Depends(get_async_session)):
